...
class DoseDistribution {
   ...
   public:
    constexpr DoseDistribution(T x, T y, T z, ThreeVector<> voxelSize)
        : m_data(x * y * z), m_access_wrapper(m_data.data(), DYNAMIC_3D_EXTENDS(x, y, z)), m_voxelSize(voxelSize) {}

    constexpr DoseDistribution(T x, T y, T z, ThreeVector<> voxelSize, T *data)
        : m_access_wrapper(m_data.data(), DYNAMIC_3D_EXTENDS(x, y, z)), m_voxelSize(voxelSize) {
        std::copy(data, data + x * y * z, std::back_inserter(m_data));
    }

    template <typename V>
    constexpr DoseDistribution(T x, T y, T z, ThreeVector<> voxelSize, V &&data)
        : m_data(data), m_access_wrapper(m_data.data(), DYNAMIC_3D_EXTENDS(x, y, z)), m_voxelSize(voxelSize) {}

    constexpr DoseDistribution() : m_data(), m_access_wrapper(m_data.data(), DYNAMIC_3D_EXTENDS()), m_voxelSize({}) {}
    ...
    constexpr ThreeVector<> voxelSize() const noexcept { return m_voxelSize; }
};
...
class HalfDistanceVoxelCube : public Geometry {
   public:
    ...
    ODPM_INLINE constexpr real_type distanceToBoundary(const Particle &particle) const noexcept {
        ...
        ThreeVector<real_type> distance{std::numeric_limits<real_type>::max(), std::numeric_limits<real_type>::max(),
    ...
    ODPM_INLINE constexpr void toGray() {
        ...
        constexpr auto to_kg_mm3   = [](const Material &material) { return material.mass_density() * 1e-6; };
