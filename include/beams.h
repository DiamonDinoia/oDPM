...
class PencilBeam {
   public:
    ...
    ODPM_INLINE ODPM_CONST constexpr T generateParticle(__attribute__((unused)) Random &rng) const noexcept {
        return k_default_particle;
    }
    ...
    ODPM_INLINE constexpr explicit ToyLinac(const real_type RZ0, const real_type energy,
        ...
        : m_rz0(RZ0),
          ...
          m_cos_min{source_axis_distance /
                    math::sqrt(math::pow(source_axis_distance, 2) + 0.5 * math::pow(fluence_field_length, 2))} {};
...
class IRCPhaseSpace {
   public:
    ...
    T generateParticle(Random &rng) const override {
        static std::uniform_int_distribution<unsigned> distribution{0, static_cast<unsigned>(m_particle_list.size())};
        unsigned trialPoint;
