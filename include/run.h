...
class Run {
   private:
    ...
    class Simulation {
       public:
        ODPM_INLINE constexpr explicit Simulation(Physics<V, Z> &physics) noexcept : physics(physics) {}
   ...
   public:
    Run(const unsigned long seed, const unsigned threads = std::thread::hardware_concurrency()) : k_threads{threads} {
    ...
    void simulate(V &geometry, const T &beam, const Material &referenceMaterial, const ODPMElectronData &electron_data,
                  const ODPMPhotonData &photon_data, std::uint64_t histories) {
...
#pragma omp parallel for default(none) firstprivate(beam) schedule(static) \
    shared(chunks, geometry, referenceMaterial, photon_data, electron_data, std::cout, histories)
        for (unsigned i = 0; i < k_threads; ++i) {
            ...
            for (unsigned history = 0; history < histories; ++history) {
