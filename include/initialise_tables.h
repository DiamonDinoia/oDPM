...
namespace opmc {
    ...
    public:
        DPMTables(const std::vector<T> &materials, const std::string &gInputDataDir = INPUT_DATA_DIR,
                  const std::string &gOutputDataDir = OUTPUT_DATA_DIR,
                  const std::string &gDataDirEnvVar = G4DATASET_DIR) :
                materialNames(saveMaterialNames(materials)),
                m_materials(initialiseMaterials(materials)) {
            ElectronData electronData(gInputDataDir,
                                      static_cast<int>(m_materials.size()),
                                      opmc::constants::k_ElectronCut * .99,
                                      opmc::constants::k_MaxEkin,
                                      opmc::constants::k_NumEkinForElectron);
            PhotonData photonData(static_cast<int>(m_materials.size()),
                                  opmc::constants::k_GammaCut * .99,
                                  opmc::constants::k_MaxEkin,
                                  opmc::constants::k_NumEkinForElectron);
    ...
    private:
        ...
        void initialiseTables(ElectronData &electronData, PhotonData &photonData,
