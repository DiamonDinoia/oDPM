...
// Possible interaction events saved as an enum instead of string or integer for clarity and debug reasons.
    enum Event {
        NOTHING = 0, BREMSSTRAHLUNG, MOLLER, HINGE, OUT_OF_BOUNDARY, ABSORBED, MICROSTEP
    };
    ...
    public:
        ...
        ODPM_INLINE void hingeInteraction(Electron &particle, real_type initialEnergy) noexcept {
            const real_type polarAngle =
                    r_reference_material.angularDeflectionElectron(initialEnergy, r_rng.getUniform(),
                                                                   r_rng.getUniform());
         ...
         * (Interaction is not possible otherwise)
         */
        ODPM_INLINE Photon bremsstrahlungInteraction(const Material &material,
                                                     Electron &electron) noexcept {
            // sample energy transferred to the emitted gamma photon
            const real_type energyLoss =
                    material.seltzerBergerElectron(electron.energy, r_rng.getUniform(), r_rng.getUniform(),
                                                   r_rng.getUniform());
        ...
        ODPM_INLINE Event electronTracking(Electron &electron, ParticleState &state) noexcept {
            ...
            real_type midStepE = math::max(std::fma(stepLength, theHalfDEDX, electron.energy),
                                           constants::k_ElectronCut);
         ...
         */
        ODPM_INLINE Event continuousElectronTracking(Electron &electron,
                                                     ParticleState &state) noexcept {
            Event event;
         ...
         */
        ODPM_INLINE constexpr void simulateStep(Electron &electron, ParticleState &state,
